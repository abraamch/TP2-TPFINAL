...
login = (req, res, next) => {
    ...
    (req, res) => {
      ...
      if (perfilCliente) {  
        ...
        res.status(401).send('Se requiere login' );
      }
    }
  );
    ...
    if (req.isAuthenticated()) {
      ...
      if (productoComprado != null) {
        ...
        if (cliente != null) {
          ...
          res.send('la compra del ' +  nombre +' se realizo con exito');
        } else {
  ...
  verCompras = (req , res ) => {
    if(req.isAuthenticated()) {
    const productos = req.user.productos
    res.send("Pokemones comprados: " + productos)
    } else {
      res.send('Debes iniciar sesión para ver tus compras');
  }  }
